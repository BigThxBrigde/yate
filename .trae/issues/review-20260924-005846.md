...
**Minor 中本轮未处理（2 条，建议单独立项）**：

- `--version` 拖入整个 TUI 栈：`diagnostics.version_lines` 依赖 `Editor` 类型，R6 禁 `TYPE_CHECKING`，
  需要把版本行下沉到叶子模块——属结构调整而非修 bug。
- F 键 dispatch 失败时行为不一致：与 vim「未映射键被吞」的既定语义耦合，改动会牵动既有测试与用户预期。
