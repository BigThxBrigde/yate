...
> **⚠️ 2026-09-26 分层重构后校准**（calibration pass，全文锚点以此为准）：
>
> 分层重构（`issues/refine-arch`，L0–L4 边界见 `.trae/rules/architecture-boundaries.md`）已改变按键链路布局。
> 本计划正文中引用的 `yate/interfaces.py`（**已删除**）、`yate/app.py:985-999` 等旧锚点一律按下表换算；
> 文中行号如与最新代码冲突，以注释/符号名为准（行号会漂移，结构不会）。
>
> | 旧锚点（已失效） | 重构后位置 | 说明 |
> |---|---|---|
> | `yate/interfaces.py:156`（`handle_raw_key` 签名） | `yate/editor.py:634`（`Editor.handle_raw_key(raw)`） | interfaces 层已按 R2 废止，扩签名改在 `Editor` 上做 |
> | `yate/app.py:985-999`（app.on_key 字符串特例） | `yate/editor.py:618-626` | **P2.4 已被重构顺带完成**：`ctrl+shift+e`/`ctrl+1`/`ctrl+p` 成为 `Editor.handle_key` 的 event.key 分支 |
> | `yate/app.py:739-744` / `:1002-1007` | `yate/editor.py:554`（`Editor.handle_key`）/ `yate/app.py:150-154`（`YateApp.on_key` 仅转发） | 派发中枢从 App 下沉到 L3 Editor，**event.key 原生可用**（P2.3 大部分已 moot） |
...
> | `yate/diagnostics.py`（sections） | 模块级 `sections` 表 | P7.2 落点 |
> | `yate/services/extensions.py:242-266` | `yate/services/extensions.py:277`（`bind_key`） | P7.4 落点 |
> | `editor_view/terminal.py:189-208` / `:33-45` | `yate/editor_view/terminal.py:211`（TOGGLE_KEYS 处理）/ `:49`（TOGGLE_KEYS 定义） | 集成终端交互 |
