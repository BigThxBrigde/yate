...
| `--readonly` | 以只读方式打开文件参数：编辑与保存被拒绝，直到 `:set readonly=false` 解锁；参数为目录时忽略 |
...
**反馈颜色约定**：每条命令的结果都会占据消息行——绿色表示成功
（`:w`、`:set …`、`:term` 等），黄色表示警告或操作被拦截
（有未保存修改时的 `:q`、未知命令、只有一个标签时的 `:bn`），红色表示错误。
打开整屏浮层的命令（`:manual`、`:help`、`:files`、`:palette`、
`:diagnostics`、Shell 命令输出）会先把消息行重置为空闲提示，
浮层关闭后上一条命令的消息不会残留重现。
...
**窗格与窗口（NORMAL 模式 `Ctrl+W` 前缀，详见 8.1 节）**

| 按键 | 功能 |
|---|---|
| `Ctrl+W` `s` / `v` | 水平分割 / 垂直分割窗格（无参数复制当前文档） |
| `Ctrl+W` `q` / `o` | 关闭活动窗格 / 只保留活动窗格（等同 `:only`） |
| `Ctrl+W` `h` `j` `k` `l` | 按方向移动到相邻窗格（`h` 可到左侧文件树） |
| `Ctrl+W` `Ctrl+W` | 在各编辑窗格与文件树之间循环 |
| `Ctrl+W` `+` / `-` | 增大 / 减小活动窗格高度 |
| `Ctrl+W` `<` / `>` | 减小 / 增大活动窗格宽度 |
| `Ctrl+W` `=` | 同组窗格等分 |
| `Ctrl+W` `h`（文件树聚焦时） | 留在文件树；`l` / `j` / `k` 返回编辑器 |
...
| 按键 | 功能 |
|---|---|
| `Ctrl+B` | 显示 / 隐藏文件树（显示时自动聚焦到树；未打开目录时侧栏自动隐藏） |
...
启动参数决定侧栏初始状态：参数是**目录**时文件树默认显示（浏览模式）；
参数是**文件**（含尚不存在的新文件路径）时文件树默认隐藏，焦点直接进入
编辑（可用 `Ctrl+B` 或 `:explorer` 随时调出，工作区根为该文件所在目录）；
不带参数启动时同样隐藏。
...
- **文件过滤**：默认隐藏 dot 开头的文件（如 `.env`、`.eslintrc`），
  按 `H` 可切换。`.git`、`.hg`、`__pycache__`、`.venv`、`node_modules` 等
  目录始终隐藏。此外，yate 会读取工作区根目录和各子目录下的
  `.gitignore` / `.yateignore` 文件，按其中的 glob 模式过滤（支持 `!` 取反、
  结尾 `/` 仅匹配目录）。在 yaterc 中设置 `show_hidden = True` 可改变默认行为。
...
### 8.1 窗格分割（`:split` / `:vsplit`）

编辑区可以像 vim 一样分割成多个窗格。每个窗格显示一个文档，并拥有**独立的
光标、选区和滚动位置**——同一文档同时显示在多个窗格中时互不干扰。

| 命令 | 别名 | 说明 |
|---|---|---|
| `:split [路径]` | `:sp` | 水平分割（上下两个窗格）；无参数时在新窗格中打开当前文档 |
| `:vsplit [路径]` | `:vs` | 垂直分割（左右两个窗格）；无参数时在新窗格中打开当前文档 |
| `:only` | — | 只保留活动窗格，关闭其余窗格（文档仍保留为标签 / 隐藏 buffer） |
| `:close` | `:cl` | 关闭活动窗格（最后一个窗格时不动作并提示用 `:q` 退出；等同 `Ctrl+W q`） |

- 带路径时在新窗格打开该文件；相对路径先相对当前文档所在目录解析，其次相对
  工作目录；参数是目录时转交文件树打开。
- vim 键位下提供 `Ctrl+W` 前缀和弦（见 5.2 节窗口表）；vsc 键位没有和弦，
  用 `F5` 命令行执行上述命令即可。
- 关闭窗格用 `:close`（别名 `:cl`）或 vim 的 `Ctrl+W q`：只关窗格不关文档
  （文档仍保留为标签 / 隐藏 buffer），最后一个窗格时不动作；关窗格不拦截未
  保存修改。
- `:q` / `:quit` **始终退出整个 yate**，即使开着多个窗格也一样（有未保存
  修改时拦截，`:q!` 放弃修改强制退出，`:wq` 保存并退出）。
...
| `:split [路径]` | `:sp` | 水平分割窗格；无参数复制当前文档（见 8.1 节） |
| `:vsplit [路径]` | `:vs` | 垂直分割窗格；无参数复制当前文档（见 8.1 节） |
| `:only` | — | 只保留活动窗格 |
| `:close` | `:cl` | 关闭活动窗格（最后一个窗格时不动作） |
...
| `:termclose` | 隐藏集成终端（Shell 进程保持运行）；提示 `terminal hidden`，本就隐藏时给出警告 |
...
| `show_hidden` | `bool` | `False` | `True` / `False` | 文件树默认是否显示 dot 开头的隐藏文件 |
...
- `` Ctrl+` `` 切换面板（反引号，Tab 上方的键）。打开时焦点进入终端，隐藏时焦点
  回到编辑器。
- **Windows 传统控制台**（conhost / 旧版 xterm）里 `` Ctrl+` `` 与 `Ctrl+Space`
  发送同一个 NUL 字节。此时该字节在编辑器里统一作为 `Ctrl+Space`（手动补全），
  不会打开终端；终端打开时仍可用它关闭。需要打开终端请用 `:term` 或命令面板。
...
- `filetypes` / `args` / `root_markers` 接受列表或元组；扩展名前导点会被
  自动去掉（`".rs"` → `"rs"`，但仅由点组成的非法值会报错）；`name` 与
  `command` 两端的空白会被去除。未识别的多余键将被忽略。
...
`:wq` 保存并退出。`:q` / `:quit` 无论开着多少窗格都直接退出整个 yate；
若只想关闭当前窗格而不退出，用 `:close` / `:cl` 或 `Ctrl+W q`（文档仍
保留在标签中，不拦截未保存修改）。
...
| `Ctrl+W` `s/v/q/o` | 横切 / 竖切 / 关窗 / 只留本窗 | `Ctrl+W` `hjkl` | 窗格间移动焦点 |
