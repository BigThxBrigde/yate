...
- vim 键位下提供 `Ctrl+W` 前缀和弦（见 5.2 节窗口表）；vsc 键位没有和弦，
  用 `F5` 命令行执行上述命令即可。
- 多窗格时 `:q` 关闭**活动窗格**（不拦截未保存修改，文档仍保留为标签）；
  只剩最后一个窗格时 `:q` 照常退出 yate（有未保存修改时拦截）。
  `:quit` 则始终直接退出整个 yate（多窗格时也一样，有未保存修改时拦截）。
  `:q!` 与 `:wq` 始终作用于退出，语义不变。
...
|---|---|---|
| `:w` | `:write` | 保存当前文件 |
| `:q` | — | 多窗格时关闭活动窗格；单窗格时退出 yate（有未保存修改时拦截，见 8.1 节） |
| `:quit` | — | 直接退出 yate（多窗格时也一样；有未保存修改时拦截，`:q!` 强制） |
| `:q!` | — | 丢弃修改并强制退出 |
| `:wq` | — | 保存并退出 |
...
`:wq` 保存并退出。注意多窗格时 `:q` / `Ctrl+W q` 只关闭活动窗格、不会
拦截未保存修改（文档仍保留在标签中），只有最后一个窗格的 `:q` 才退出；
`:quit` 则无论多少窗格都直接退出整个 yate。
