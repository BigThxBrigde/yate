...
Release history: [CHANGELOG.md](CHANGELOG.md) · [中文](CHANGELOG.zh.md) — generated by `python -m tools.changelog`. Read it at runtime with `yate --changelog [en|zh]` or the `:changelog [en|zh]` command inside the editor.
...
| `Ctrl+1` | focus the editor (matches VS Code; all terminals on Windows via the chord driver — kitty/CSI-u only elsewhere) |
