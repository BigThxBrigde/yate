...
async def _type_save_find(tmp: Path) -> ScenarioResult:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        await pilot.pause()
        checks.append(Check("buffer[0]", "hello", app.editor.session.buffer.lines[0]))
        ...
        await pilot.pause()
        mode = (app.editor.prompt_bar.active_mode
                if app.editor.prompt_bar else None)
        checks.append(Check("find_mode", "find", mode))
        await pilot.press("l", "l")
        await pilot.press("enter")
        await pilot.pause()
        checks.append(Check("search_query", "ll", app.editor.session.search.query))
        checks.append(Check("search_matches>=1", True,
                            len(app.editor.session.search.matches) >= 1))
...
async def _file_palette(tmp: Path) -> ScenarioResult:
    ...
    async with app.run_test(size=(110, 32)) as pilot:
        ...
        await pilot.press("enter")
        await pilot.pause()
        checks.append(Check("doc.name", "notes.txt", app.editor.session.doc.name))
...
async def _keymap_toggle(tmp: Path) -> ScenarioResult:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        await pilot.pause()
        checks.append(Check("default_keymap", "vsc", app.editor.keymaps.name))
        await pilot.press("ctrl+/")
        await pilot.pause()
        checks.append(Check("toggled_keymap", "vim", app.editor.keymaps.name))
        await pilot.press("ctrl+/")
        await pilot.pause()
        checks.append(Check("back_to_vsc", "vsc", app.editor.keymaps.name))
        rows = snapshot_svg(app, tmp)
