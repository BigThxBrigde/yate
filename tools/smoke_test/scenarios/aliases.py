...
async def _action_alias_roundup(tmp: Path) -> ScenarioResult:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        await pilot.pause()
        checks.append(Check("delete_forward", "ello world", app.editor.session.buffer.lines[0]))
        # alt+d deletes the rest of the word
        await pilot.press("alt+d")
        await pilot.pause()
        checks.append(Check("delete_word_fwd", " world", app.editor.session.buffer.lines[0]))
        # ctrl+shift+right selects one word to the right
        await pilot.press("ctrl+shift+right")
        await pilot.pause()
        checks.append(Check("select_word_right", True, app.editor.session.buffer.has_selection()))
        await pilot.press("escape")
        await pilot.pause()
        # shift+up / shift+down extend the selection by a line
        await pilot.press("enter")
        await pilot.pause()
        checks.append(Check("two_lines", 2, app.editor.session.buffer.line_count))
        await pilot.press("shift+up")
        await pilot.pause()
        checks.append(Check("select_up", True, app.editor.session.buffer.has_selection()))
        await pilot.press("escape")
        await pilot.pause()
        await pilot.press("shift+down")
        await pilot.pause()
        checks.append(Check("select_down", True, app.editor.session.buffer.has_selection()))
        await pilot.press("escape")
        await pilot.pause()
...
async def _command_alias_roundup(tmp: Path) -> ScenarioResult:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        checks.append(Check("enew", 2, len(app.editor.session.docs)))
        await run_command(pilot, "bd")
        checks.append(Check("bd", 1, len(app.editor.session.docs)))
        ...
        checks.append(Check("cl_noop", 1, app.editor.panes.leaf_count
                            if app.editor.panes else 0))
        await run_command(pilot, "welcome")
        checks.append(Check("welcome", True, app.editor.session.welcome_visible))
